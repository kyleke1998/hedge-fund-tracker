...
pipenv run gen-correlation                  # rebuild database/correlation.csv (QQQ realized average stock correlation)
pipenv run gen-momentum                     # rebuild database/momentum.csv (SPY three-month rate of change)
...
- **`app/analysis/correlation.py`** — realized average stock correlation for the `/regime` page: the mean of every pairwise correlation of daily returns across a **fixed** Nasdaq-100 basket over a rolling 63-session window. QQQ's own bars define the session calendar; a name missing any session inside a window is dropped from that window (never forward-filled — a carried price reads as a flat day), and a session most of the then-listed basket has no bar for is treated as a hole in the price source and removed before returns are taken. Compute is offline → `scripts/gen_correlation.py` writes the CSV; the page only reads it.
- **`app/analysis/momentum.py`** — SPY three-month rate of change for the `/regime` page: `(price_t / price_{t−63} − 1) × 100` on `bar_history` typical prices, already put on today's split basis by the loader so a split in the window never reads as a crash or a spike. One ticker, one 63-session lookback, warm-up sessions before `HISTORY_START` loaded for the first window but never emitted. Compute is offline → `scripts/gen_momentum.py` writes the CSV; the page only reads it.
...
- `src/lib/data/correlation.ts` — the correlation series loader plus its two pure axis helpers (`yearTicks`, `correlationTicks`), used by the Realized average stock correlation panel on `/regime`
- `src/lib/data/momentum.ts` — the SPY three-month rate-of-change series loader plus its pure axis helpers (`yearTicks`, `rocTicks`), used by the S&P 500 three-month rate of change panel on `/regime`
...
- **`regime.csv`** — regime signals in **long format**: one row per (`kind`, `quarter`, `key`, `value`), where kind is `metric` (position churn, capital recycling, untouched share, turnover), `mega` (basket weight + chain-linked split-adjusted share index) or `sector` (weights as % of classified value, with the broad multi-theme sectors split into named subsectors — Semiconductors, Software, Biotech, … — per `SUBSECTOR_BY_INDUSTRY` in `app/analysis/regime.py`). Regenerated by `pipenv run gen-regime`; bundled to GH Pages and read by the `/regime` page.
- **`correlation.csv`** — realized average stock correlation, one row per session (`date`, `correlation`, `stocks`). `stocks` is how many basket names the reading came from, and it grows over the record as constituents list. Regenerated by `pipenv run gen-correlation`; bundled to GH Pages and read by the `/regime` page.
- **`momentum.csv`** — SPY three-month rate of change, one row per session (`date`, `roc`), where `roc` is the trailing 63-session percent change. Regenerated by `pipenv run gen-momentum`; bundled to GH Pages and read by the `/regime` page.
